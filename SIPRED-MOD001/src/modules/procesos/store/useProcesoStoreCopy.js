...
import {getProcess,setProcess,updateProcess,deleteProcess,} from '@mod1/services/processService.js'
  ...
  actions: {
    ...
    async fetchProceso() {
      try {
        ...
        //console.log('useProcesoStore fetchProceso datos '+this.proceso.values);
        this.listaProceso = this.proceso.map((val) => ({
          ...
          activo: val.activo,
          ruta: val.ruta,      
          sistema: val.sistemaId, // placeholder until backend provides it
        ...
        this.rowsProceso = this.proceso.map((val) => ({
          id : val.id,
          nombre: val.descr,
          Sistema: val.sistema.nombre, // placeholder until backend provides it
          tipo: val.tipo == "A" ? 'AGRUPADOR' : 'PROCESO',
          icono: val.icono,
          activo: val.activo,
          ruta: val.ruta,         
