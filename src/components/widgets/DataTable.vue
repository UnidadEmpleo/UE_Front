<template>
  <div class=" py-3 container-fluid" ref="rootRef">
    <div class="mt-4 row">
      <div class="col-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="justify-content-between">
              ...
              <div v-if="updatable">
                  <material-button color="primary" variant="gradient" size="sm"  class="me-2" >
                    {{ updatableTitle }}
                  </material-button>
              </div>
            </div>
              ...
              <div v-if="searchable" class="search-container flex-grow-1">
                <div v-if="showAdvancedSearch" class="d-flex align-items-center gap-2">
                  <select v-model="selectedFilter" class="form-select" style="max-width:160px;">
                    <option v-for="opt in internalFilterOptions" :key="opt.value" :value="opt.value">{{ opt.label }}
                    </option>
                  </select>
                  <input ref="searchInputRef" v-model="searchQuery" type="text" class="form-control" autofocus
                    placeholder="Buscar..." @keyup.enter="triggerSearch" />
                  <material-button color="info" variant="gradient" @click="triggerSearch">Buscar</material-button>
                </div>
                <input v-else ref="searchInputRef" v-model="searchQuery" type="text" class="form-control" autofocus
                  placeholder="Buscar..." />
              </div>
              <!-- custom header actions slot (e.g., Buscar button) -->
              <slot name="header-actions"></slot>
              <div v-if="exportar">
                <material-button v-permiso="permisoExportar" color="info" variant="gradient" @click="downloadCSV">
                  Descargar CSV
                </material-button>
              </div>
            </div>
          ...
          <div class="table-responsive position-relative " style="height: 52vh;">
            ...
            <table :id="tableId" class="table table-flush " style="height: 90%;"
              :class="{ 'table-disabled': isLoading }">
              <thead class="thead-light">
                <tr>
                  <th v-if="actionsPosition === 'start' && $slots['row-actions']" :class="[
                  ...
                  <th v-if="actionsPosition === 'end' && $slots['row-actions']" :class="[
                    'text-uppercase text-secondary text-xxs font-weight-bolder opacity-7',
              ...
              <tbody>
                <tr v-for="(row, rowIndex) in paginatedRows" :key="rowIndex" :class="row._rowClass || ''">
                  ...
                  <td v-if="actionsPosition === 'start' && $slots['row-actions']" :class="[
                  ...
                  <td v-for="(column, colIndex) in columns" :key="colIndex" class="text-sm font-weight-normal">
                    <template v-if="$slots[column] || $slots[column.replace(/\s+/g, '_')]">
                      <slot :name="$slots[column] ? column : column.replace(/\s+/g, '_')" :row="row"
                        :value="row[column]" :index="rowIndex"></slot>
                    </template>
                    <template v-else-if="column === 'select'">
                      <input :id="`chk-${row.id}`" type="checkbox" :checked="row.id === selectedId" class="me-2"
                        @change="onRowSelect(row, $event.target.checked)" />
                    </template>
                    <template v-else-if="column === 'activo'">
                      <span :class="row[column] ? 'tag tag-green' : 'tag tag-red'" class="fw-bold shadow-sm">
                  ...
                  <td v-if="actionsPosition === 'end' && $slots['row-actions']" :class="[
                    'text-sm font-weight-normal',
                    hasStickyActions ? 'sticky-column sticky-column-right' : ''
                  ]">
            ...
            <!-- Paginación estilo quads -->

          </div>

          <div class="">
            <div v-if="uiTotalPages > 1 && showFooter" class="pagination-dots-wrapper">
              <button class="circle-btn nav-btn" :disabled="currentPage === 1 || isLoading" title="Página anterior"
                @click="prevPage"><span class="arrow left"></span></button>
              <div class="page-dots">
                <button v-for="p in visiblePages" :key="'pg-' + p" class="dot-btn"
                  :class="{ active: p === currentPage }" :disabled="isLoading" :title="'Ir a página ' + p"
                  @click="goToPage(p)">{{ p }}</button>
              </div>
              <button class="circle-btn nav-btn" :disabled="currentPage === uiTotalPages || isLoading"
                title="Página siguiente" @click="nextPage"><span class="arrow right"></span></button>
            </div>
          </div>
