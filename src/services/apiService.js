    ...
    try {
        ...
        try {
          if (error.response.data instanceof Blob) {
            ...
            try {
              ...
              error.response.data = parsed;// Sobrescribe el data del response para reutilizar la lógica existente
            } catch {
              error.response.data = { message: text };// No es JSON, usar texto plano
            }
          }
        } catch (e) {
      ...
      // Error con respuesta del servidor
        if(error.response.status === 404)
          message = error.response?.data || 'No encontrado';
        ...
        else
        {
          ...
          if(data != null && !data?.data && error.response.status === 400)
          {
            ...
            if (customAlert) {mainStore.triggerAlert({message,color: 'warning',icon: 'error',})}
